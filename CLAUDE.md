...
## 0. Claude Code Prompting with Guides & Tokens

### Guides & Design Tokens are for **topic-wise Claude prompting only**

When building a composition in Claude Code:

1. **Read the guide** (`guides/history/documentary.md` or topic-specific)
   - Contains scene templates (TplEditorialHeadline, TplStatCallout, etc.)
   - Documents components (KineticText, Stamp, GoldDivider, etc.)
   - Explains animation engine (prog(), lerp(), easing)
   - Defines typography scale and color rules

2. **Load design tokens** (`designs/claude/claude.tokens.json`)
   ```json
   { "colors": { "primary": "#cc785c", ... },
     "typography": { "displayXl": { "size": "64px", ... } },
     "spacing": { "xl": "32px" } }
   ```

3. **Use guides + tokens to build composition**
   - Never hardcode hex values — use `TOKEN.*` constants from tokens file
   - Never guess templates — use scene selection logic from guide
   - Never invent components — use documented components from guide
   - Match each sentence's `beat` field to appropriate template

### Example Claude Code Prompt

```
You are building a Remotion composition for a YouTube Short.

Guide: guides/history/documentary.md
Tokens: designs/claude/claude.tokens.json
Script: data/output/script.json (19 sentences, each with beat, highlightWords, dataValue)

Task: Map each sentence to a scene template:
- beat="hook" → TplEditorialHeadline (bold, large, BgFlare)
- beat="build" → TplTextDominant or TplSplitPhotoData
- dataValue != null → TplStatCallout (giant number)
- Content is timeline → TplTimeline
- Content is process → TplFlowDiagram

Use TOKEN.colors, TOKEN.typography, TOKEN.spacing from tokens file.
Build small, modular scene components (not one giant file).
```
...
A short-form video generation pipeline. Given a topic brief, it produces a
publication-ready `.mp4` vertical video (1080×1920, 9:16) using guides and design tokens.
...
Stage 1: Script Generation
  User Brief → (pipeline.ts) → data/output/script.json + public/voice.mp3 + out/prompt/claude.md

Stage 2: Composition Build  (Claude Code — reads out/prompt/claude.md)
  script.json → src/scenes/<video-name>/** + src/compositions/<video-name>/**

Stage 3: Video Rendering
  src/compositions/<video-name>/ → (Remotion) → out/video.mp4

Stage 4: Audio Stitch (Optional)
  out/video.mp4 + public/voice.mp3 → (ffmpeg) → out/video_with_audio.mp4
```

**Audio is generated by the pipeline automatically** when `ELEVENLABS_API_KEY` is set.
Skip it with `--skip-audio`. Re-generate at any time: `npm run gen:audio`

### Key Constraints

- **Modular components**: Break compositions into small, reusable scene files (not one giant file)
- **Design-driven**: Use `guides/` and `designs/` to prompt Claude, never hardcode styling
- **Guide + tokens**: Both files are input to Claude prompting, not burned into code
- **Topic-wise**: Each topic can have its own guide (guides/history/, guides/science/, etc.)
...
├── AGENTS.md                              ← Instructions for Claude Code composition agents
├── package.json                           ← Single package (not a monorepo)
├── tsconfig.json                          ← TS config, strict mode
├── remotion.config.ts                     ← Remotion bundler config
├── pipeline.ts                            ← Pipeline: script → audio → out/prompt/claude.md
├── data/
│   ├── input/input.txt                    ← User topic brief
│   └── output/
│       ├── script.json                    ← Generated script (sentences, beats, accents)
│       └── output.txt                     ← Plain narration text for TTS
│
├── out/
│   └── prompt/claude.md                   ← Handoff prompt for Claude Code agent
│
├── lmstudio/                              ← LLM orchestration (LM Studio / OpenAI-compat)
│   ├── orchestrate.ts                     ← generateVideoSpec() — Call 1 + save outputs
│   ├── call1.ts                           ← Script writer (structured JSON output)
│   ├── caller.ts                          ← HTTP client for LM Studio API
│   ├── types.ts                           ← All types (ScriptPackage, VideoSpec, TokenMap…)
│   ├── constants.ts                       ← ANIMATION_TYPES, ACCENT_PALETTE
│   ├── validate.ts                        ← Visual directive validation
│   └── prompts/
│       └── prompt_call1_script.txt        ← LLM prompt — loaded at runtime, never hardcoded
│
├── elevenlabs/                            ← TTS client
│   ├── generate.ts                        ← Audio generation + word-level timestamps
│   └── index.ts                           ← Public exports (generateWithTimestamps, WordTiming)
│
├── public/                                ← Static assets for Remotion
│   ├── timing.json                        ← Word timings JSON (from ElevenLabs)
│   ├── voice.mp3                          ← TTS audio file
│   └── clips/                             ← B-roll video clips
│
├── designs/
│   └── claude/claude.tokens.json          ← Design tokens (colors, typography, spacing)
│
├── guides/                                ← Category-specific visual guides for prompting
│   └── history/documentary.md
│
...
└── src/                                   ← Remotion compositions + scene engine
    ...
    ├── Root.tsx                           ← <Composition> registration — imports from video index files
    ├── context/
    │   └── PaletteContext.tsx             ← Palette context + usePalette() hook
    ├── animations/                        ← SHARED animation components (reusable across videos)
    │   ├── registry.ts
    │   ├── Counter.tsx, LineChart.tsx, … (one file per type)
    │   └── data/world-countries.json
    ├── schemas/
    │   └── animationSpec.zod.ts
    ├── compositions/
    │   └── <video-name>/                  ← ONE folder per video (kebab-case slug)
...
| Per-video `compositions/<name>/` folder | Keeps each video self-contained. Prevents accidental coupling between videos. |
| Required `index.ts` per video folder | Clean import surface. `Root.tsx` never reaches into deeply nested files. |
| No video-specific code in `animations/` or `lib/` | Shared dirs must stay reusable. Video-specific helpers inside the video folder. |


---

## 15. Video Folder Structure — Rules with No Exceptions

This section governs how Claude Code agents must organise generated code.
Violating these rules will break the import graph and cause render failures.

### 15.1 One Folder Per Video

Every video gets exactly one folder under `compositions/` and one under `scenes/`.
The folder name is a **kebab-case slug** derived from the topic (e.g. `tunguska-1908`, `black-death`).

```
src/compositions/<video-name>/      ← composition entry point
src/scenes/<video-name>/            ← all scene components for this video
```

**Never** place video files directly in `src/compositions/` or `src/scenes/` root.

### 15.2 Required index.ts in Every Video Folder

Every `compositions/<video-name>/` and `scenes/<video-name>/` folder **must** contain an `index.ts`:

```typescript
// src/compositions/tunguska-1908/index.ts
export { TunguskaComposition } from "./TunguskaComposition";

// src/scenes/tunguska-1908/index.ts
export { HookScene }  from "./HookScene";
export { BodyScene }  from "./BodyScene";
export { CloseScene } from "./CloseScene";
```

`src/Root.tsx` imports **only** from these index files — never from nested files:

```typescript
// Good ✅
import { TunguskaComposition } from "./compositions/tunguska-1908/index";

// Bad ❌
import { TunguskaComposition } from "./compositions/tunguska-1908/TunguskaComposition";
```

### 15.3 Shared vs Video-Specific — Decision Table

| Location | Contains | Rule |
|---|---|---|
| `src/animations/` | Reusable animation components (Counter, LineChart, …) | Only if used by 2+ videos |
| `src/lib/` | Pure utility functions | Only if used by 2+ videos |
| `src/components/` | Generic UI components (KineticText, etc.) | Only if used by 2+ videos |
| `src/context/` | React contexts and hooks | Only global contexts |
| `src/compositions/<video-name>/` | Composition entry + helpers for this video only | Always video-specific |
| `src/scenes/<video-name>/` | Scene components for this video only | Always video-specific |

If a helper is used by exactly one video, it lives **inside that video's folder**, not in `lib/`.

### 15.4 Prompt Files — No Hardcoded Strings

All LLM prompt text must live in `.txt` files. No exceptions.

```
lmstudio/prompts/prompt_call1_script.txt   ← script generation prompt
data/prompts/<video-name>/system.txt       ← video-specific system prompt (if needed)
```
...
const prompt = fs.readFileSync(
  path.join(__dirname, "../lmstudio/prompts/prompt_call1_script.txt"), "utf-8"
);

// Bad ❌ — hardcoded prompt string in .ts source
const prompt = `You are an art director for a documentary...`;
```

### 15.5 File-Creation Checklist (Before Every New File)

```
1. Is this a prompt string?
   Yes → Create a .txt file. Stop.

2. Will this code be used by more than one video?
   Yes → src/animations/, src/lib/, src/components/, or src/context/
   No  → src/compositions/<video-name>/ or src/scenes/<video-name>/

3. Is this the composition entry point or a scene?
   Composition → src/compositions/<video-name>/<VideoName>Composition.tsx
   Scene       → src/scenes/<video-name>/<SceneName>Scene.tsx

4. Did I create or update the index.ts for that folder?
   No → Do it now before committing.

5. Did I import into Root.tsx from the index file (not a nested file)?
