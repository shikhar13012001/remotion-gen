# Composition Brief: Model Context Protocol

Build `ModelContextProtocolComposition` for a 1080x1920 Remotion short.
...
- Sentences: 16
- Accent: `#ff385c`
- Audio: `public/voice.mp3` and `public/timing.json` are available.

## Read First

- `data/output/script.json` - sentence text, beats, durations, highlightWords, dataValue, needsImage.
...
- `guides/science/general.md` - category guide; this is the design and scene-template authority.
...
- Create video-specific code only under `src/compositions/model-context-protocol/` and `src/scenes/model-context-protocol/`.
- Add `index.ts` files for both folders and import the composition in `src/Root.tsx` only from `./compositions/model-context-protocol/index`.
...
- Use image backgrounds only when they add place or evidence. Check `guides/science/bg` first, then `public/bg-image.png`.
- Prefer fetched assets in `public/assets/model-context-protocol/` when `image_manifest.json` provides them. Respect the sentence's intended visualQuery usage: background plate, split panel, subject cutout, or annotated evidence. When a query has multiple fetched assets, choose the one that best fits the scene layout instead of reusing the background by default.
...
- `node_modules\\.bin\\remotion.cmd render ModelContextProtocolComposition out\\video.mp4` on Windows, or `npx remotion render ModelContextProtocolComposition out/video.mp4`.
