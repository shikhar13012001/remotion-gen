...
export function buildHandoffPrompt(opts: HandoffPromptOptions): string {
  ...
  const guideLine = guidePath
    ? `- \`${guidePath}\` — category guide; scene-template and visual-language authority.`
    : "- No guide path provided. Infer conservatively from script metadata and generated art direction.";
  ...
  const audioLine = opts.hasAudio
    ? "`public/voice.mp3` and `public/timing.json` are available. Sync every scene cut to `timing.json` word boundaries — never cut mid-word."
    : "Audio is not available yet. Derive all scene durations strictly from `suggested_duration_ms` in the script. Do not hardcode frame counts.";

  return `# Composition Brief: ${opts.topic}

Build \`${componentName}\` for a 1080×1920 Remotion short.
...
### File Structure
- All video-specific code lives exclusively under \`src/compositions/${slug}/\` and \`src/scenes/${slug}/\`.
- Add \`index.ts\` to both folders.
- Import the composition in \`src/Root.tsx\` only from \`./compositions/${slug}/index\`.
...
- Check \`${bgDir}\` first, then \`public/bg-image.png\`.
- Prefer fetched assets in \`public/assets/${slug}/\` when \`image_manifest.json\` provides them.
...
\`\`\`
# Windows
node_modules\\.bin\\remotion.cmd render ${componentName} out\\video.mp4

# Unix
npx remotion render ${componentName} out/video.mp4
\`\`\`
